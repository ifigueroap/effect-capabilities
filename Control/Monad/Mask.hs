...
type TStateTP capability s m = Tagged capability (StateTP capability s m)

runTStateTP :: (Monad m) => s -> TStateTP k s m a -> m (a, s)
runTStateTP s m = runStateTP (unTag m) s

evalTStateTP :: (Monad m) => s -> TStateTP k s m a -> m a
evalTStateTP s m = evalStateTP (unTag m) s

type TErrorTP c e m = Tagged c (ErrorTP c e m)

runTErrorTP :: (Monad m, Error e) => TErrorTP c e m a -> m (Either e a)
runTErrorTP m = runErrorTP (unTag m)
